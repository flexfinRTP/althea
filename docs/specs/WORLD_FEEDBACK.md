...
Code integration is in the repository:

* `@worldcoin/idkit` / `@worldcoin/idkit-core` 4.2.x
* `selfieCheckLegacy`
* RP signatures in `/api/world/rp-signature`
* `POST https://developer.world.org/api/v4/verify/{rp_id}` from `/api/world/verify`
* Manual Review path on `/case/:id/verify`

Live World Developer Portal configuration and Sandbox Selfie Check have **not** been run. Remaining `[FILL AFTER TEST]`, `[FILL AFTER USER TEST]`, and portal-access items stay unmarked until Justin completes a real sandbox session. Do not invent runtime bugs.
...
```text
Not yet observed in the live Developer Portal. Selfie Check access is gated
(developers@toolsforhumanity.com). Official docs used for implementation:
https://docs.world.org/world-id/idkit/integrate
https://docs.world.org/world-id/idkit/credentials
https://docs.world.org/world-id/sandbox/testing-selfie-check
```
...
```text
Not yet observed in the live Developer Portal. Documentation search was used
during implementation; Sandbox/portal search remains untested.
```
...
```text
Official docs distinguish Selfie Check from World ID proof and AgentKit.
Althea selected Selfie Check only as a Relief-fund liveness signal.
Portal product-discovery UX is untested until access is granted.
```
...
```text
Docs cover client vs verify API vs credentials. Live failure isolation
(client / Sandbox / backend / configuration) is untested without portal access.
Althea therefore keeps a Manual Review path and does not fake proofs.
```
...
```text
Blocked on Selfie Check feature access and World Developer Portal credentials.
Expected steps from docs: create app, configure RP, request Selfie Check access,
use staging environment. Actual step count and delays: not yet measured.
```
...
```text
Not observed. Do not list invented states.
```
...
```text
Implemented in code, not yet executed against Sandbox:

1. Frontend requests RP signature from /api/world/rp-signature.
2. IDKitRequestWidget opens with selfieCheckLegacy.
3. Unmodified IDKit result posts to /api/world/verify.
4. Backend calls World /api/v4/verify/{rp_id}.
5. Persisted status is passed | failed | manual_review. No selfie stored.
```
...
```text
No live Sandbox error catalog yet. Client fallback copy:
